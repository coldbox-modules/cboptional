...
component {
	...
	cbOptional function of( required value ) {
		if ( isNull( arguments.value ) ) {
			throw( message = "Cannot set a null value using the of() method", type = "NullPointerException" );
		}
	...
	cbOptional function filter( required predicate ) {
		if ( this.isPresent() ) {
			return ( arguments.predicate( this.get() ) ? this : this.empty() );
		}
	...
	numeric function hashCode() {
		return ( this.isPresent() ? createObject( "java", "java.lang.System" ).identityHashCode( variables.value ) : 0 );
	}
	...
	String function toString() {
		return ( this.isPresent() ? variables.value.toString() : "" );
	}
